...
/**
 * 每日任务 Created by 赵俊 on 2019/7/23 10:54.
 */
@Slf4j
public class DailyTask implements ILogicModule {
	private static final String RECORD_DAILY_TASK = "DailyTaskList";// 今天玩家需要完成的任务
	private static final String PROPERTY_ASSIGN_TASK_LAST_TIME = PLAYER_PROPERTY_DAILYTASKASSIGNLASTTIME;// 上次任务分配时间
	private static final String PROPERTY_DAILY_TASK_POINTS = PLAYER_PROPERTY_DAILYTASKPOINTS;// 每日任务积分
	public static final String PROPERTY_HAS_AWARDED_POINTS = PLAYER_PROPERTY_DAILYTASKHASAWARDEDPOINTS;// 已经领取额外奖励时的积分
	...
	public boolean onInit(IKernel kernel) {
		...
		// 请求领取奖励
		kernel.regClientMessage(C2SMsgDef.C2S_GET_DAILY_TASK_AWARD.ordinal(), this, "OnRequestGetReward");
		...
		kernel.listenPropertyChange(PROPERTY_DAILY_TASK_POINTS, "Player", this, "OnPropertyChange");
		...
		XML dailyTask = new XML("res/Task/DailyTask.xml", null, kernel, (iKernel, cfg) -> loadDailyTask(iKernel, cfg));
		m_parseXML = new XML("res/Task/DailyTaskExtraAwards.xml", dailyTask, kernel,
				(iKernel, cfg) -> loadDailyTaskExtraAwards(iKernel, cfg));
		return true;
	}
	...
	public void OnPlayerClassCreate(IKernel kernel, String script) {
		...
		kernel.declareProperty(script, PROPERTY_DAILY_TASK_POINTS, ValueType.INT, false, true, true);
		kernel.declareProperty(script, PROPERTY_HAS_AWARDED_POINTS, ValueType.INT, false, true, true);
		IRecord rec = kernel.declareRecord(script, RECORD_DAILY_TASK, TaskCol.COL_END.ordinal(), m_mapAllTasks.size(),
				false, true, true);
	...
	public void OnPropertyChange(IKernel kernel, IGameObject player, String name, Object oldValue) {
		int currentPoints = player.getInt(PROPERTY_DAILY_TASK_POINTS);
		if (PROPERTY_DAILY_TASK_POINTS.equals(name) && currentPoints - (int) oldValue > 0) {
				...
				if (currentPoints >= points && points > hasAwarded) {
					...
					if (award != null) {
						...
						m_itemModule.AddItem(kernel, player, award.itemId, award.count,
								UtilFunc.System.DAILY_TASK.ordinal(), "daily task get");
	...
	public void OnRequestGetReward(IKernel kernel, IGameObject player, int msgid, byte[] msg)
		...
		IRecord rec = player.getRecord(RECORD_DAILY_TASK);
...
//					,rec.getString(i, 3));
	...
	private void loadDailyTask(IKernel kernel, ICfgReader cfg) {
	...
	private void loadDailyTaskExtraAwards(IKernel kernel, ICfgReader cfg) {
	...
	private void checkDailyTask(IKernel kernel, IGameObject player) {
		long current = UtilFunc.getZeroTime(kernel.getServerTime());
		if (player.getLong(PROPERTY_ASSIGN_TASK_LAST_TIME) != current) {
			player.setProperty(PROPERTY_ASSIGN_TASK_LAST_TIME, current);
			player.setProperty(PROPERTY_DAILY_TASK_POINTS, 0);
			player.setProperty(PROPERTY_HAS_AWARDED_POINTS, 0);
			IRecord record = player.getRecord(RECORD_DAILY_TASK);
